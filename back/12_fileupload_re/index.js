...
//정적파일 설정
//__dirname : 현재 파일 경로
//서버실행 시 http://localhost:8000/uploads/파일명
...
//key-value 에서 key 값과 value의 변수가 동일하면 합칠 수 있음
...
//멀티(ver1)
app.post('/uploads/array', upload.array("userfiles", 2), (req, res) => {
    console.log(req.files)
    res.send(req.body)
})

//멀티(ver2)
//maxCount : 업로드할 최대 파일 갯수 설정
...
//페이지를 지정할 때는 미들웨어 use를 사용한다
//use는 http 전송 방식을 이해하지 못한다
//같은 url로 get,post를 만들 수 있지만 use로는 접근이 힘듦
//예를 들어 get 방식의 '/login'과 post 방식의 '/login'은 다른 페이지이지만 use는 동일한 페이지로 인식
